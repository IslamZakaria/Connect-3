...
class Gaming:
    ...
    def AlphaBeta(self, board, depth, alpha, beta, currentPlayer):
        ...
        GameOver = self.is_terminal_node(board)
        ...
        if currentPlayer: # Maximizing player
            for col in valid_locations:
                row = self.get_next_open_row(board, col)
                BoardCopy = board.copy()
                self.drop_piece(BoardCopy, row, col, self.AI_PIECE)
            ...
            for col in valid_locations:
                row = self.get_next_open_row(board, col)
                BoardCopy = board.copy()
                self.drop_piece(BoardCopy, row, col, self.PLAYER_PIECE)
